...
from backend.rag.benchmarks.qasper.ablation import QASPER_ABLATION_VARIANTS
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the AITrans QASPER query-time RAG ablation matrix."
    )
    ...
    parser.add_argument(
        "--variants",
        nargs="+",
        choices=tuple(item.variant_id for item in QASPER_ABLATION_VARIANTS),
        default=[item.variant_id for item in QASPER_ABLATION_VARIANTS],
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    query_planner = (
        ...
        if any(
            item.variant_id in args.variants and item.multi_query
            for item in QASPER_ABLATION_VARIANTS
        )
        else None
    )
    try:
