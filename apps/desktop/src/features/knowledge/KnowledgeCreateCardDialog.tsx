...
import { newKnowledgeCardMetadata } from "./knowledge-card-model"
...
const choices: Array<{ type: CreatableType; label: string; icon: typeof StickyNote; description: string }> = [
  ...
  { type: "evidence", label: "Evidence", icon: Quote, description: "Preserve a claim, observation, or source-backed piece of evidence." },
  { type: "insight", label: "Insight", icon: Sparkles, description: "Record a synthesis or conclusion derived from your knowledge." },
  { type: "question", label: "Question", icon: CircleHelp, description: "Keep an open research question that can guide later work." },
  { type: "highlight", label: "Highlight", icon: Highlighter, description: "Save a concise quote, finding, or important passage." },
  { type: "paper", label: "Paper", icon: BookOpenText, description: "Create a manual paper card without attaching a local file." },
]
...
function KnowledgeCreateCardDialogContent({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/25 p-4 backdrop-blur-[2px]" role="presentation" onMouseDown={onClose}>
      <section className="ait-scroll-panel max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-[22px] border border-white/60 bg-white p-5 shadow-2xl" role="dialog" aria-modal="true" aria-label="Create knowledge card" onMouseDown={(event) => event.stopPropagation()}>
        <header className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-slate-400">Knowledge card</p><h3 className="mt-1.5 text-lg font-semibold text-slate-950">Create a new card</h3><p className="mt-1 text-xs leading-5 text-slate-500">Use one card model for notes, concepts, evidence, insights, questions, highlights, and papers.</p></div><Button variant="ghost" size="xs" aria-label="Close create card dialog" onClick={onClose}><X size={16} /></Button></header>
        ...
        <footer className="mt-5 flex justify-end gap-2"><Button variant="ghost" disabled={creating} onClick={onClose}>Cancel</Button><Button variant="primary" disabled={!canCreate} onClick={() => onCreate({ item_type: type, title: title.trim(), summary: summary.trim(), metadata: newKnowledgeCardMetadata("user") })}>{creating ? "Creating…" : "Create card"}</Button></footer>
      </section>
    </div>
  )
}
