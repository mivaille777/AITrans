...
const itemLabels: Record<KnowledgeItemType, string> = {
  paper: "Paper",
  note: "Note",
  concept: "Concept",
  highlight: "Highlight",
    ...
    <article className="group flex min-h-52 flex-col rounded-[18px] border border-slate-200/80 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.02)] transition hover:-translate-y-0.5 hover:border-slate-300 hover:shadow-[0_16px_30px_rgba(15,23,42,0.06)]">
      ...
      <button type="button" className="mt-3 text-left" onClick={onOpen}>
        <div className="flex flex-wrap items-center gap-1.5">
          <Badge tone={item.item_type === "paper" ? "info" : "neutral"}>{itemLabels[item.item_type]}</Badge>
      ...
      <div className="mt-auto pt-4 text-[10px] text-slate-400">
        ...
        ) : document?.indexed_at ? `Indexed ${new Date(document.indexed_at).toLocaleDateString()}` : `Updated ${new Date(item.updated_at).toLocaleDateString()}`}
      </div>
    </article>
  )
}
