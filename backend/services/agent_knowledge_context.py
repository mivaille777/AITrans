...
def normalize_knowledge_context(value: Any) -> dict[str, Any]:
    ...
    The public API validates this structure, but runtime adapters may also call
    this helper with plain dicts or Pydantic models. The normalized result is
    intentionally bounded and contains only Canvas/card/relation metadata that
    is safe to expose to planner and synthesis prompts.
    """
...
def compact_knowledge_context(value: Any, *, max_chars: int = 10_000) -> dict[str, Any]:
    """Return prompt-safe Canvas context while preserving relations first.

    Relations are admitted before card summaries so a large Canvas cannot push
    its explicit edges past a tail-truncation boundary. The returned object is
    always valid JSON and fits within ``max_chars`` when serialized.
    """
